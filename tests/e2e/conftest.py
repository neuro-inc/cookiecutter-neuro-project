...
def cookiecutter_setup(change_directory_to_temp: None) -> Iterator[None]:
    run(f"cookiecutter --no-input {PATH_ROOT} project_name='{PROJECT_NAME}'")
...
def neuro_login() -> None:
    ...
    proc = run(f"neuro config login-with-token {token} {url}")
    assert f"Logged into {url}" in proc.stdout, proc
    run(f"neuro config switch-cluster {cluster}")
    run("neuro config show")


def run(cmd: str, assert_exit_code: bool = True) -> "subprocess.CompletedProcess[str]":
